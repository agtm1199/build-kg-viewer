...
const CypherResultTable = ({ data, ...props }) => {
  ...
  const rowContainsElement = useCallback(
    (row, elId) => {
      if (!elId) return false;
      return Object.values(row).some(
        (val) => val && typeof val === 'object'
          && String(val.id) === String(elId),
      );
    },
    [],
  );

  const getFirstNodeId = useCallback((row) => {
  ...
  return (
    <div className="legend-area">
      ...
      <Table
        ...
        rowClassName={(record) => (
          rowContainsElement(record, selectedElementId)
            ? 'table-row-synced' : ''
        )}
        onRow={(record) => ({
          onClick: () => {
            if (onSelectElement) {
              const nid = getFirstNodeId(record);
              if (nid) onSelectElement(nid);
            }
          },
...
CypherResultTable.defaultProps = {
