...
import QueryLoadingOverlay from '../../cytoscape/QueryLoadingOverlay';
...
const CypherResultFrame = ({
  ...
  const [thicknessOpenCount, setThicknessOpenCount] = useState(0);
  ...
  useEffect(() => {
    ...
    if (chartAreaRef.current && thicknessOpenCount > 0) {
  ...
  }, [filterModalVisible, thicknessOpenCount, chartLegend]);
  ...
  const downloadTsv = () => {
    ...
    const dataJson = eleJson.map((ele) => ({
      ...
      source: ele.data.source,
      target: ele.data.target,
      properties: JSON.stringify(ele.data.properties),
    }));
    const headers = Object.keys(dataJson[0]).join('\t');
    const rows = dataJson.map((r) => Object.values(r).join('\t')).join('\n');
    const tsv = `${headers}\n${rows}`;
    saveAs(
      new Blob([`\uFEFF${tsv}`], { type: 'text/tab-separated-values;charset=utf-8' }),
      `${reqString.replace(/ /g, '_')}.tsv`,
    );
  };
  ...
  return (
    <>
      <Frame
        ...
        onThick={() => setThicknessOpenCount((c) => c + 1)}
        thicnessMenu={
          (
            <EdgeThicknessMenu
              onSubmit={(thicness) => {
        ...
        onDownload={(type) => {
          ...
          else if (type === 'png') downloadPng();
          else if (type === 'tsv') downloadTsv();
        }}
        executionTime={queryComplete?.executionTime}
        reqString={reqString}
        isPinned={isPinned}
        refKey={refKey}
        isTable={isTable}
      >
        {
          queryComplete?.complete
            ...
            : (
              <QueryLoadingOverlay
                reqString={reqString}
                onCancel={() => dispatch(removeActiveRequests(refKey))}
              />
            )
        }
      </Frame>
