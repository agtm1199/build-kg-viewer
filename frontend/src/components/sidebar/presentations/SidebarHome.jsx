...
import {
  VerticalLine, SubLabelLeft, SubLabelRight,
...
const NodeList = ({ nodes, setCommand }) => {
  if (!nodes || nodes.length === 0) {
    return (
      ...
      >
        <span style={{ fontSize: '11px', color: '#9CA3AF' }}>No node labels found</span>
      </div>
    );
  }
...
const EdgeList = ({ edges, setCommand }) => {
  if (!edges || edges.length === 0) {
    return (
      ...
      >
        <span style={{ fontSize: '11px', color: '#9CA3AF' }}>No edge labels found</span>
      </div>
    );
  }
...
const PropertyList = ({ propertyKeys, setCommand }) => {
  if (!propertyKeys || propertyKeys.length === 0) {
    return (
      ...
      >
        <span style={{ fontSize: '11px', color: '#9CA3AF' }}>No properties found</span>
      </div>
    );
  }
  const list = propertyKeys.map((item) => (
    <PropertyItems
      key={uuid()}
...
const SidebarHome = ({
  ...
  propertyKeys,
  setCommand,
  command, // eslint-disable-line no-unused-vars
  trimFrame, // eslint-disable-line no-unused-vars
  addFrame, // eslint-disable-line no-unused-vars
  getMetaData,
  changeCurrentGraph,
  changeGraph,
  isLabel,
  ...
  return (
    <div className="sidebar-home">
      ...
      {/* Refresh/Disconnect/Graph dropdown moved to top toolbar */}
    </div>
  );
};
