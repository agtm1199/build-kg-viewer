...
export const nodeLabelColors = [
  {
    color: '#F97316', borderColor: '#EA580C', fontColor: '#FFFFFF', nodeLabels: new Set([]), index: 0,
  },
  {
    color: '#498EDA', borderColor: '#2870C2', fontColor: '#FFFFFF', nodeLabels: new Set([]), index: 1,
  },
  {
    color: '#8DCC93', borderColor: '#5DB665', fontColor: '#2A2C34', nodeLabels: new Set([]), index: 2,
  },
  {
    color: '#DA7194', borderColor: '#CC3C6C', fontColor: '#FFFFFF', nodeLabels: new Set([]), index: 3,
  },
  {
    color: '#FFC454', borderColor: '#D7A013', fontColor: '#2A2C34', nodeLabels: new Set([]), index: 4,
  },
  {
    color: '#57C7E3', borderColor: '#23B3D7', fontColor: '#2A2C34', nodeLabels: new Set([]), index: 5,
  },
  {
    color: '#F16667', borderColor: '#EB2728', fontColor: '#FFFFFF', nodeLabels: new Set([]), index: 6,
  },
  {
    color: '#D9C8AE', borderColor: '#C0A378', fontColor: '#2A2C34', nodeLabels: new Set([]), index: 7,
  },
  {
    color: '#C990C0', borderColor: '#B261A5', fontColor: '#FFFFFF', nodeLabels: new Set([]), index: 8,
  },
  {
    color: '#569480', borderColor: '#447666', fontColor: '#FFFFFF', nodeLabels: new Set([]), index: 9,
  },
  {
    color: '#F79767', borderColor: '#F36924', fontColor: '#2A2C34', nodeLabels: new Set([]), index: 10,
  },
  {
    color: '#604A0E', borderColor: '#423204', fontColor: '#FFFFFF', nodeLabels: new Set([]), index: 11,
  },
];
...
export const edgeLabelColors = [
  ...
  {
    color: '#57C7E3', borderColor: '#23B3D7', fontColor: '#2A2C34', edgeLabels: new Set([]), index: 1,
  },
  {
    color: '#F79767', borderColor: '#F36924', fontColor: '#2A2C34', edgeLabels: new Set([]), index: 2,
  },
  {
    color: '#C990C0', borderColor: '#B261A5', fontColor: '#2A2C34', edgeLabels: new Set([]), index: 3,
  },
  {
    color: '#F16667', borderColor: '#EB2728', fontColor: '#2A2C34', edgeLabels: new Set([]), index: 4,
  },
  {
    color: '#8DCC93', borderColor: '#5DB665', fontColor: '#2A2C34', edgeLabels: new Set([]), index: 5,
  },
  {
    color: '#D9C8AE', borderColor: '#C0A378', fontColor: '#2A2C34', edgeLabels: new Set([]), index: 6,
  },
  {
    color: '#498EDA', borderColor: '#2870C2', fontColor: '#FFFFFF', edgeLabels: new Set([]), index: 7,
  },
  {
    color: '#ECB5C9', borderColor: '#DA7298', fontColor: '#2A2C34', edgeLabels: new Set([]), index: 8,
  },
  {
    color: '#FFC454', borderColor: '#D7A013', fontColor: '#2A2C34', edgeLabels: new Set([]), index: 9,
  },
  {
    color: '#DA7194', borderColor: '#CC3C6C', fontColor: '#FFFFFF', edgeLabels: new Set([]), index: 10,
  },
  {
    color: '#569480', borderColor: '#447666', fontColor: '#FFFFFF', edgeLabels: new Set([]), index: 11,
  },
];
...
let nextNodeColorIndex = 0;
const nodeColorCache = new Map();

const getNodeColor = (labelName) => {
  if (nodeColorCache.has(labelName)) {
    return nodeColorCache.get(labelName);
  }
  // Check existing assignments
  for (let i = 0; i < nodeLabelColors.length; i += 1) {
    if (nodeLabelColors[i].nodeLabels.has(labelName)) {
      const c = {
        color: nodeLabelColors[i].color,
        borderColor: nodeLabelColors[i].borderColor,
        fontColor: nodeLabelColors[i].fontColor,
      };
      nodeColorCache.set(labelName, c);
      return c;
    }
  }
  const idx = nextNodeColorIndex % nodeLabelColors.length;
  nextNodeColorIndex += 1;
  nodeLabelColors[idx].nodeLabels.add(labelName);
  const c = {
    color: nodeLabelColors[idx].color,
    borderColor: nodeLabelColors[idx].borderColor,
    fontColor: nodeLabelColors[idx].fontColor,
  };
  nodeColorCache.set(labelName, c);
  return c;
};

let nextEdgeColorIndex = 0;
const edgeColorCache = new Map();

const getEdgeColor = (labelName) => {
  if (edgeColorCache.has(labelName)) {
    return edgeColorCache.get(labelName);
  }
  for (let i = 0; i < edgeLabelColors.length; i += 1) {
    if (edgeLabelColors[i].edgeLabels.has(labelName)) {
      const c = {
        color: edgeLabelColors[i].color,
        borderColor: edgeLabelColors[i].borderColor,
        fontColor: edgeLabelColors[i].fontColor,
      };
      edgeColorCache.set(labelName, c);
      return c;
    }
  }
  const idx = nextEdgeColorIndex % edgeLabelColors.length;
  nextEdgeColorIndex += 1;
  edgeLabelColors[idx].edgeLabels.add(labelName);
  const c = {
    color: edgeLabelColors[idx].color,
    borderColor: edgeLabelColors[idx].borderColor,
    fontColor: edgeLabelColors[idx].fontColor,
  };
  edgeColorCache.set(labelName, c);
  return c;
};
...
export const updateLabelColor = (labelType, labelName, newLabelColor) => {
  if (labelType === 'node') {
    ...
    nodeColorCache.delete(labelName);
  } else {
    ...
    edgeColorCache.delete(labelName);
  }
};
...
export const generateCytoscapeElement = (data, maxDataOfGraph, isNew) => {
  ...
  if (data) {
    const rows = (maxDataOfGraph > 0)
      ? data.slice(0, maxDataOfGraph)
      : data;
    rows.forEach((row) => {
